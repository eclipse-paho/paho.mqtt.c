...
static int (*handleSSLObjectAtConnect)(SSL*) = NULL;
...
{
	int rc = 0;
	int sslCallbackReturnedValue = 1;
	...
	{
		if(handleSSLObjectAtConnect)
		{
			sslCallbackReturnedValue = handleSSLObjectAtConnect(ssl);
		}
	...
	if (sslCallbackReturnedValue != 1)
	{
		FUNC_EXIT_RC(sslCallbackReturnedValue);
		return sslCallbackReturnedValue;
	}
	FUNC_EXIT_RC(rc);
	return rc;
}
...
void SSLPluginInterface_setcallback(int(*callback)(SSL*))
{
	handleSSLObjectAtConnect = callback;
}
#endif
